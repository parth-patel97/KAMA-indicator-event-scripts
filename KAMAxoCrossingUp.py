...
def ReadData():
    ...
    if file_name_market2 != None and len(file_name_market2) > 0:
        global df2
        df2 = pd.read_csv(
            file_name_market2, delimiter=",", index_col="Date", parse_dates=True
        )
    ...
    if file_name_market3 != None and len(file_name_market3) > 0:
        global df3
        df3 = pd.read_csv(
            file_name_market3, delimiter=",", index_col="Date", parse_dates=True
        )
    ...
    if file_name_Vix != None and len(file_name_Vix) > 0:
        global dfV
        dfV = pd.read_csv(
            file_name_Vix, delimiter=",", index_col="Date", parse_dates=True
        )
...
def GetCustomSignal():
    ...
    Signal = [
        1
        if df1["Close"][i] >= df1["KAMA"][i]
        and df1["Close"][i - 1] < df1["KAMA"][i - 1]
        else 0
        for i, m in enumerate(df1["KAMA"])
    ]
    df1["Signal"] = Signal
